...
void stop_execution(sig) int sig;
{
  had_sigint = TRUE;
  printf("\n");
...
unsigned char byte(pc) program_counter *pc;
...
void push_constant(in_char, conv_base) char (*in_char)(VOID);
int conv_base;
{
  int digits;
...
void push_b10_const(pc) program_counter *pc;
{
  bc_num build;
...
void assign(c_code) char c_code;
{
