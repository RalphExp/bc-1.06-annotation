...
id_rec *find_id(tree, id) id_rec *tree;
char *id;
{
  int cmp_result;
...
int insert_id_rec(root, new_id) id_rec **root;
id_rec *new_id;
{
  id_rec *A, *B;
...
int lookup(name, namekind) char *name;
int namekind;
{
  id_rec *id;
...
void warranty(prefix) char *prefix;
{
  printf("\n%s", prefix);
  show_bc_version();
  printf(
...
#endif
#else
void yyerror(str, va_alist) char *str;
#endif
{
  char *name;
  va_list args;
...
void warn(char *mesg, ...)
#else
void warn(mesg) char *mesg;
#endif
#else
void warn(mesg, va_alist) char *mesg;
#endif
{
  char *name;
  va_list args;
...
#else
void rt_error(mesg) char *mesg;
#endif
#else
void rt_error(mesg, va_alist) char *mesg;
#endif
{
  va_list args;

  fprintf(stderr, "Runtime error (func=%s, adr=%d): ", f_names[pc.pc_func],
          pc.pc_addr);
#ifndef VARARGS
  va_start(args, mesg);
#else
...
#else
void rt_warn(mesg) char *mesg;
#endif
#else
void rt_warn(mesg, va_alist) char *mesg;
#endif
{
  va_list args;

  fprintf(stderr, "Runtime warning (func=%s, adr=%d): ", f_names[pc.pc_func],
          pc.pc_addr);
#ifndef VARARGS
  va_start(args, mesg);
#else
